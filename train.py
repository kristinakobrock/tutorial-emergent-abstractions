...
def main(params):
    ...
    if opts.shapes3d:
        data_set_name = '3dshapes/shapes3d_feat_rep'
        ...
        if opts.zero_shot:
            ...
            if opts.zero_shot_test is not None:
                ...
                if not opts.load_dataset:
                    if opts.shapes3d:
                        data_set = load_or_create_dataset('./dataset/feat_rep_zero_concept_dataset_'
                                                          + opts.zero_shot_test, device=opts.device,
                                                          game_size=opts.game_size, zero_shot=opts.zero_shot,
            ...
            # or both test conditions are generated        
                ...
                for cond in ['generic', 'specific']:
                    ...
                    if not os.path.exists(opts.save_path) and opts.save:
                        os.makedirs(opts.save_path)
                    if opts.shapes3d:
                        data_set = load_or_create_dataset('./dataset/feat_rep_zero_concept_dataset_' + cond,
                                                          device=opts.device, game_size=opts.game_size,
                                                          zero_shot=opts.zero_shot, zero_shot_test=opts.zero_shot_test)
                    else:
                        data_set = dataset.DataSet(opts.dimensions,
                                                   game_size=opts.game_size,
                                                   scaling_factor=opts.scaling_factor,
